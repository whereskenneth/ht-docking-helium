/*
 ...
 $Id: printEnergies.cc,v 1.2 2010/04/01 15:37:13 pkcoff Exp $
...
#pragma omp threadprivate (threadID, nb_group_energy)
...
void print_molar_buffer(double x) {
    // 1e-3  <= x < 1     mM millimolar
    // 1e-6  <= x < 1e-3  uM micromolar
    // 1e-9  <= x < 1e-6  nM nanomolar
    // 1e-12 <= x < 1e-9  pM picomolar
    ...
    //          x < 1e-24    sub-yottomolar
    if ((fabs((x)) > 1e-3) && ((fabs(x)) <= 1.)) {
        sprintf(searchResultsLine[threadID],"%7.2f mM (millimolar)", x*1e3);
   	strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);
    } else if ((fabs((x)) > 1e-6) && ((fabs(x)) <= 1e-3)) {
        sprintf(searchResultsLine[threadID],"%7.2f uM (micromolar)", x*1e6);
   	strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);
    } else if ((fabs((x)) > 1e-9) && ((fabs(x)) <= 1e-6)) {
        sprintf(searchResultsLine[threadID],"%7.2f nM (nanomolar)", x*1e9);
   	strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);
    } else if ((fabs((x)) > 1e-12) && ((fabs(x)) <= 1e-9)) {
        sprintf(searchResultsLine[threadID],"%7.2f pM (picomolar)", x*1e12);
   	strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);
    } else if ((fabs((x)) > 1e-15) && ((fabs(x)) <= 1e-12)) {
        sprintf(searchResultsLine[threadID],"%7.2f fM (femtomolar)", x*1e15);
   	strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);
    } else if ((fabs((x)) > 1e-18) && ((fabs(x)) <= 1e-15)) {
        sprintf(searchResultsLine[threadID],"%7.2f aM (attomolar)", x*1e18);
   	strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);
    } else if ((fabs((x)) > 1e-21) && ((fabs(x)) <= 1e-18)) {
        sprintf(searchResultsLine[threadID],"%7.2f zM (zeptomolar)", x*1e21);
   	strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);
    } else if ((fabs((x)) > 1e-24) && ((fabs(x)) <= 1e-21)) {
        sprintf(searchResultsLine[threadID],"%7.2f yM (yottomolar)", x*1e24);
   	strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);
    } else {
        sprintf(searchResultsLine[threadID],((fabs((x)) >= 0.0) && ((fabs(x)) <= 1000.)) ? "%+7.2f" : "%+11.2e" , (x));
   	strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);
    }
}
...

{
    ...
    if (strncmp(prefixString, "UNBOUND", 7) != 0 ) {
        sprintf(searchResultsLine[threadID],"%sEstimated Free Energy of Binding    = ", prefixString);
        strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);

        sprintf(searchResultsLine[threadID],((fabs((eb->deltaG)) >= 0.0) && ((fabs(eb->deltaG)) <= 1000.)) ? "%+7.2f" : "%+11.2e" , (eb->deltaG));
   	strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);

        sprintf(searchResultsLine[threadID]," kcal/mol  [=(1)+(2)+(3)-(4)]\n");
   	strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);

        if (eb->deltaG < 0.0) {
            if (ligand_is_inhibitor == 1) {
 	        sprintf(searchResultsLine[threadID],"%sEstimated Inhibition Constant, Ki   = ", prefixString);
 	        strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);
           } else {
	        sprintf(searchResultsLine[threadID],"%sEstimated Dissociation Constant, Kd = ", prefixString);
   	        strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);
            }
            // print1000_no_sign(logFile, Ki);
            print_molar_buffer(Ki);
            sprintf(searchResultsLine[threadID],"  [Temperature = %.2f K]\n", TK);
 	    strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);
        }

        sprintf(searchResultsLine[threadID],"%s\n", prefixString);
 	strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);
    }

    sprintf(searchResultsLine[threadID],"%s(1) Final Intermolecular Energy     = ", prefixString);
    strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);

    sprintf(searchResultsLine[threadID],((fabs((eb->e_inter)) >= 0.0) && ((fabs(eb->e_inter)) <= 1000.)) ? "%+7.2f" : "%+11.2e" , (eb->e_inter));
    strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);

    
    sprintf(searchResultsLine[threadID]," kcal/mol\n%s    vdW + Hbond + desolv Energy     = ",prefixString);
    strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);
    
    sprintf(searchResultsLine[threadID],((fabs((emap_total)) >= 0.0) && ((fabs(emap_total)) <= 1000.)) ? "%+7.2f" : "%+11.2e" , (emap_total));
    strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);
    
        
    sprintf(searchResultsLine[threadID]," kcal/mol\n%s    Electrostatic Energy     = ",prefixString);
    strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);
    
    sprintf(searchResultsLine[threadID],((fabs((elec_total)) >= 0.0) && ((fabs(elec_total)) <= 1000.)) ? "%+7.2f" : "%+11.2e" , (elec_total));
    strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);
    ...
    if (B_have_flexible_residues) {
        sprintf(searchResultsLine[threadID],"%s    Moving Ligand-Fixed Receptor    = ", prefixString);
        strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);
                
        sprintf(searchResultsLine[threadID],((fabs((eb->e_inter_moving_fixed)) >= 0.0) && ((fabs(eb->e_inter_moving_fixed)) <= 1000.)) ? "%+7.2f" : "%+11.2e" , (eb->e_inter_moving_fixed));
        strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);
        
        sprintf(searchResultsLine[threadID]," kcal/mol\n%s    Moving Ligand-Moving Receptor     = ",prefixString);
        strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);
    
	sprintf(searchResultsLine[threadID],((fabs((eb->e_inter_moving_moving)) >= 0.0) && ((fabs(eb->e_inter_moving_moving)) <= 1000.)) ? "%+7.2f" : "%+11.2e" , (eb->e_inter_moving_moving));
        strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);
	sprintf(searchResultsLine[threadID]," kcal/mol\n");
        strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);
    }

    sprintf(searchResultsLine[threadID],"%s(2) Final Total Internal Energy     = ", prefixString); 
    strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);
    
    sprintf(searchResultsLine[threadID],((fabs((eb->e_intra)) >= 0.0) && ((fabs(eb->e_intra)) <= 1000.)) ? "%+7.2f" : "%+11.2e" , (eb->e_intra));
    strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);
	...
	sprintf(searchResultsLine[threadID],"%s    Internal Energy Ligand          = ", prefixString); 
        strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);
        
    sprintf(searchResultsLine[threadID],((fabs((eb->e_intra_lig)) >= 0.0) && ((fabs(eb->e_intra_lig)) <= 1000.)) ? "%+7.2f" : "%+11.2e" , (eb->e_intra_lig));
    strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);
	sprintf(searchResultsLine[threadID]," kcal/mol\n");
        strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);

    sprintf(searchResultsLine[threadID],((fabs((eb->e_intra_rec)) >= 0.0) && ((fabs(eb->e_intra_rec)) <= 1000.)) ? "%+7.2f" : "%+11.2e" , (eb->e_intra_rec));
    strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);

        sprintf(searchResultsLine[threadID]," kcal/mol\n%s    Internal Moving-Fixed Receptor     = ",prefixString);
        strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);

        sprintf(searchResultsLine[threadID],((fabs((eb->e_intra_moving_fixed_rec)) >= 0.0) && ((fabs(eb->e_intra_moving_fixed_rec)) <= 1000.)) ? "%+7.2f" : "%+11.2e" , (eb->e_intra_moving_fixed_rec));
    strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);
        
        
        
        sprintf(searchResultsLine[threadID]," kcal/mol\n%s    Internal Moving-Moving Receptor     = ",prefixString);
        strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);


        sprintf(searchResultsLine[threadID],((fabs((eb->e_intra_moving_moving_rec)) >= 0.0) && ((fabs(eb->e_intra_moving_moving_rec)) <= 1000.)) ? "%+7.2f" : "%+11.2e" , (eb->e_intra_moving_moving_rec));
    strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);
	...
	sprintf(searchResultsLine[threadID],"%s(3) Torsional Free Energy           = ", prefixString); 
        strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);
    
    sprintf(searchResultsLine[threadID],((fabs((eb->e_torsFreeEnergy)) >= 0.0) && ((fabs(eb->e_torsFreeEnergy)) <= 1000.)) ? "%+7.2f" : "%+11.2e" , (eb->e_torsFreeEnergy));
    strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);
    ...
    switch(ad4_unbound_model){
        ...
        case Unbound_Same_As_Bound:
        default:
	sprintf(searchResultsLine[threadID],"%s(4) Unbound System's Energy  [=(2)] = ", prefixString); 
        strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);
	...
	sprintf(searchResultsLine[threadID],"%s(4) Unbound System's Energy         = ", prefixString); 
        strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);

            break;
    }
    sprintf(searchResultsLine[threadID],((fabs((eb->e_unbound_internal_FE)) >= 0.0) && ((fabs(eb->e_unbound_internal_FE)) <= 1000.)) ? "%+7.2f" : "%+11.2e" , (eb->e_unbound_internal_FE));
    strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);
