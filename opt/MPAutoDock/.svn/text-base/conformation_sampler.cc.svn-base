/*
 ...
 $Id: conformation_sampler.cc,v 1.2 2010/04/01 15:37:13 pkcoff Exp $
...
// pkcoff: avoid name collisions with main static data
Real (*vtConfSamp)[SPACE], (*crdpdb)[SPACE];
int (*tlistConfSamp)[MAX_ATOMS];
Real *lig_centerConfSamp;
int natomConfSamp;
int *typeConfSamp;
GridMapSetInfo *infoConfSamp;

// pkcoff: this crd object is declared static so it can be thread-private in main.cc, on BGQ the /bgsys/drivers/ppcfloor/gnu-linux/bin/powerpc64-bgq-linux-g++ linker is catching this
// Real crd[MAX_ATOMS][SPACE];
Real crdConfSamp[MAX_ATOMS][SPACE];

Real ref_crdConfSamp[MAX_ATOMS][SPACE];

//pkcoff - openmp for bluegene change section - start
// these global variables must be thread private

#pragma omp threadprivate (vtConfSamp,crdpdb,tlistConfSamp, lig_centerConfSamp, natomConfSamp, typeConfSamp, infoConfSamp, ref_crdConfSamp)
...
ConformationSampler::ConformationSampler(State init_state) {
	...
	base_ind = set_ind(infoConfSamp, init_state);
	base_point = base_ind.phenotyp;	
	base_energy = base_point.evaluate(Normal_Eval);
	cnv_state_to_coords(init_state, vtConfSamp, tlistConfSamp, init_state.ntor, crdpdb, base_crd, natomConfSamp);
...
Real ConformationSampler::current_rmsd(void) {
	...
	cnv_state_to_coords(probe_state, vtConfSamp, tlistConfSamp, probe_state.ntor, crdpdb, crdConfSamp, natomConfSamp);
	return getrms(crdConfSamp, base_crd, RMSD_SYMMETRY, natomConfSamp, typeConfSamp);
}

Real ConformationSampler::reference_rmsd(void) {
	return getrms(base_crd, ref_crdConfSamp, RMSD_SYMMETRY, natomConfSamp, typeConfSamp);
}
...
void systematic_conformation_sampler(State hist[MAX_RUNS], int nconf, Real init_vt[MAX_TORS][SPACE], Real init_crdpdb[MAX_ATOMS][SPACE], int init_tlist[MAX_TORS][MAX_ATOMS], Real init_lig_center[SPACE], int init_natom, int init_type[MAX_ATOMS], GridMapSetInfo *init_info) {
	vtConfSamp = init_vt;
	crdpdb = init_crdpdb;
	tlistConfSamp = init_tlist;
	lig_centerConfSamp = init_lig_center;
	natomConfSamp = init_natom;
	typeConfSamp = init_type;
	infoConfSamp = init_info;
...
void setup_reference_coordinates(void) {
	for (int i = 0;  i < natomConfSamp;  i++) {
		ref_crdConfSamp[i][0] = lig_centerConfSamp[0] + crdpdb[i][0];
		ref_crdConfSamp[i][1] = lig_centerConfSamp[1] + crdpdb[i][1];
		ref_crdConfSamp[i][2] = lig_centerConfSamp[2] + crdpdb[i][2];
	}
}
