/*
 ...
 $Id: writePDBQT.cc,v 1.2 2010/04/01 15:37:13 pkcoff Exp $
...
#pragma omp threadprivate (threadID,Nnb_array,nb_group_energy)
...

{
	...
	if (outlev > -1) {
        ...
        sprintf(searchResultsLine[threadID],"%s: MODEL     %4d\n", state_type_string, irun+1 );
        strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);
        sprintf(searchResultsLine[threadID],"%s: USER    Run = %d\n", state_type_string, irun+1 );
        strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);
        sprintf(searchResultsLine[threadID],"%s: USER    DPF = %s\n", state_type_string, dpfFN );
        strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);
        sprintf(searchResultsLine[threadID],"%s: USER  \n", state_type_string );
        strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);

        printEnergies_buffer( &eb, state_type_prefix_USER_string, ligand_is_inhibitor, emap_total, elec_total, B_have_flexible_residues, ad4_unbound_model);
        ...
        sprintf(searchResultsLine[threadID],"%s: USER    NEWDPF move %s\n", state_type_string, smFileName);
        strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);


        sprintf(searchResultsLine[threadID],"%s: USER    NEWDPF about %f %f %f\n", state_type_string, sml_center[X], sml_center[Y], sml_center[Z]);
        strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);

        sprintf(searchResultsLine[threadID],"%s: USER    NEWDPF tran0 %f %f %f\n", state_type_string, state.T.x, state.T.y, state.T.z);
        strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);

        sprintf(searchResultsLine[threadID],"%s: USER    NEWDPF quaternion0 %f %f %f %f\n", state_type_string, state.Q.x, state.Q.y, state.Q.z, state.Q.w);
        state.Q = convertQuatToRot( state.Q );
        strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);

        sprintf(searchResultsLine[threadID],"%s: USER    NEWDPF axisangle0 %f %f %f %f\n", state_type_string, state.Q.nx, state.Q.ny, state.Q.nz, RadiansToDegrees(WrpRad(ModRad(state.Q.ang))));
        strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);


        sprintf(searchResultsLine[threadID],"%s: USER    NEWDPF quat0 %f %f %f %f # deprecated\n", state_type_string, state.Q.nx, state.Q.ny, state.Q.nz, RadiansToDegrees(WrpRad(ModRad(state.Q.ang))));
        strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);
        ...
        sprintf(searchResultsLine[threadID],"%s: USER    NEWDPF dihe0 ", state_type_string);
        strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);
			
			for (i = 0; i < ntor; i++) {

        sprintf(searchResultsLine[threadID],"%.2f ", RadiansToDegrees(WrpRad(ModRad(state.tor[i]))));
        strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);
       ...
       sprintf(searchResultsLine[threadID], "\n");
        ...
        sprintf(searchResultsLine[threadID], FORMAT_PDBQT_ATOM_RESSTR, state_type_prefix_string, 
        ...
        sprintf(searchResultsLine[threadID], FORMAT_PDBQT_ATOM_RESNUM, state_type_prefix_string, 
        ...
        strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);
                ...
                if (print_header) {
        sprintf(searchResultsLine[threadID],"%s: USER                              x       y       z     vdW  Elec       q    Type\n", state_type_string);
        strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);
                    
 
         sprintf(searchResultsLine[threadID],"%s: USER                           _______ _______ _______ _____ _____    ______ ____\n", state_type_string);
         strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);
 ...
 // Make sure we only print the header once
        ...
        sprintf(searchResultsLine[threadID],"%s%s", state_type_prefix_string, PDBQT_record[r]);
         strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);

            }
        } // r

        sprintf(searchResultsLine[threadID],"%s: TER\n%s: ENDMDL\n", state_type_string,state_type_string);
         strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);
