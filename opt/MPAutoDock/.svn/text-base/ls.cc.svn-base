/*
 ...
 $Id: ls.cc,v 1.2 2010/04/01 15:37:13 pkcoff Exp $
...
Boole Solis_Wets::SW(Phenotype &vector)
{
   ...
   int i,j;
   register unsigned int num_successes = 0, num_failures = 0;
...
#pragma omp threadprivate (thisQuat, prevQuat, startQuat)
...
Boole Pseudo_Solis_Wets::SW(Phenotype &vector)
{
   ...
   int i,j;
   register unsigned int num_successes = 0, num_failures = 0,  all_rho_stepsizes_too_small = 1;
...
// printf("first SUCCESS for iter %d and threadID %d\n",i,threadID);
         num_successes++;
         num_failures = 0;
         vector = newPh;
         modified = TRUE;
         ...
         if (newPh.evaluate(Normal_Eval) < vector.evaluate(Normal_Eval)) {
// printf("opposite SUCCESS for iter %d and threadID %d\n",i,threadID);
            num_successes++;
            num_failures = 0;
            vector = newPh;
            modified = TRUE;
...
#pragma omp threadprivate (thisQuat)
