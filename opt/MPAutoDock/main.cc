/* AutoDock
 $Id: main.cc,v 1.9 2010/08/05 19:17:06 pkcoff Exp $
...
//#define AUTODOCK_MPI // pkcoff: flag for autodock MPI driver for BlueGene
// #define DEBUG
...
#ifdef AUTODOCK_MPI
int mpi_slave_main(int argc, char ** argv,int mpirank, int seedtype, int mapusagetype);
#endif
...
static const char* const ident[] = {ident[1], "@(#)$Id: main.cc,v 1.9 2010/08/05 19:17:06 pkcoff Exp $"};
...
// verify params
  if (argc != 6) {
    fprintf(stderr,"autodock4: incorrect number of parameters, parameters should be: docking_list_file docking_base_directory status_directory seed_type map_file_usage\n");
    exit(-1);
  }
...
// determine seed type
if (strcmp(argv[4],"same_seed") == 0)
  seedType = SAME_SEED;
else if (strcmp(argv[4],"default_seed") == 0)
  seedType = DEFAULT_SEED;
else if (strcmp(argv[4],"unique_node_seed") == 0)
  seedType = UNIQUE_NODE_SEED;
else {
    fprintf(stderr,"autodock4: seed type unrecognized - should be one of these values: same_seed, default_seed, or unique_node_seed.\n");
    exit(-1);
}
...
if (strcmp(argv[5],"reload_maps") == 0)
  mapUsageType = RELOAD_MAPS;
else if (strcmp(argv[5],"reuse_maps") == 0)
  mapUsageType = REUSE_MAPS;
else {
    fprintf(stderr,"autodock4: map file usage unrecognized - should be one of these values: reload_maps or reuse_maps.\n");
    exit(-1);
}
...
// initialize mpi
  ...
  if (mpirank == 0) { // master node
    ...
    if ( (dockingListFile = ad_fopen(argv[1], "r")) == NULL ) {
      fprintf(stderr, "\nautodock4 master: can't find or open docking list file %s\n", argv[1]);
      exit(-1);
    }
...
// open the docking list and all the status files
    ...
    if ( (dockSubmittedStatusFile = ad_fopen(dockSubmittedStatusFileName, "w")) == NULL ) {
    ...
    if ( (dockPerformanceFile = ad_fopen(dockPerformanceFileName, "w")) == NULL ) {
      fprintf(stderr, "\nautodock4 master: can't open %s\n", dockPerformanceFileName);
      exit(-1);
    }
    
    // print out the column names in the performance file    
    pr(dockPerformanceFile,"Base Name,Node,Master Send Date,Master Send Time,Master Recv Time,Master Send-Recv Duration (Sec),Slave Send - Master Recv Delta (MPI Time) (Sec), Slave Recv Time,Slave Main Start Time,First FLD File Load Start Time,Last FLD File Load End Time,First Map File Load Start Time,Last Map File Load End Time,First Ligand File Load Start Time,Last Ligand File Load End Time,LGA Setup Start Time,LGA Start Time,LGA End Time,Analysis Start Time,Analysis End Time,Post Slave Main Cleanup Time,Slave Main End Time,Slave Send Time,Slave Main Duration (Sec),FLD File Load Total Duration (Sec),FLD File Load Number,FLD File Load Min Duration (Sec),FLD File Load Avg Duration (Sec),FLD File Load Max Duration (Sec),Map File Load Total Duration (Sec),Map File Load Number,Map File Load Min Duration (Sec),Map File Load Avg Duration (Sec),Map File Load Max Duration (Sec),Ligand File Load Total Duration (Sec),LGA Overall Duration (Sec),Number LGA Runs,LGA Run Comp Min Duration (Sec),LGA Run Comp Avg Duration (Sec),LGA Run Comp Max Duration (Sec),LGA Run IO Min Duration (Sec),LGA Run IO Avg Duration (Sec),LGA Run IO Max Duration (Sec),Analysis Duration (Sec)\n");
    ...
    if ( (dockErrorStatusFile = ad_fopen(dockErrorStatusFileName, "w")) == NULL ) {
    ...
    if ( (dockSuccessStatusFile = ad_fopen(dockSuccessStatusFileName, "w")) == NULL ) {
...
// remove eol
      ...
      strcpy(masterDockInfo.baseName,line);
	...
	dockingCounter++;
      ...
      mpirc = MPI_Send(&masterDockInfo, sizeof(dockinfo), MPI_BYTE, dockingCounter, mpitag, MPI_COMM_WORLD);

      pr(dockSubmittedStatusFile,"%s submitted to node %d at ", line,dockingCounter);

      printdate( dockSubmittedStatusFile, 1 );
      (void) fflush(dockSubmittedStatusFile);

#ifdef DEBUG
      printf("Master first loop: sent to slave %d, mpirc is %d\n",dockingCounter,mpirc);
#endif /* DEBUG */
  ...
  printf("in slave %d, waiting\n",mpirank);
    ...
    int endSlave = 0;
    while (!endSlave) { // keep waiting for send from master until the master sends an end
      mpirc = MPI_Recv(&slaveDockInfo, sizeof(dockinfo), MPI_BYTE, 0, mpitag, MPI_COMM_WORLD, &mpistatus);
...
// create the argument list that the original serial main expected from the dockinfo structure that was sent from the main
      ...
      char *mpi_slave_argv[5];
      for (int i=0;i<5;i++)
        mpi_slave_argv[i] = (char *) malloc(300*sizeof(char));

      strcpy (mpi_slave_argv[0],"autodock4");
      strcpy (mpi_slave_argv[1],"-p");
      strcpy (mpi_slave_argv[2],slaveDockInfo.baseName);
      strcat(mpi_slave_argv[2],".dpf");
      strcpy (mpi_slave_argv[3],"-l");
      strcpy (mpi_slave_argv[4],slaveDockInfo.baseName);
      strcat(mpi_slave_argv[4],".dlg");
...
// change to the diretory for the ligand to be docked
      ...
      if (chdir_rc != 0) {
        printf("change dir failed with %d errno is %d\n",chdir_rc,errno);
        perror("failed chdir");
      }
...
try {
    slaveDockInfo.dockingSuccess = mpi_slave_main(5,mpi_slave_argv,mpirank,seedType,mapUsageType);
...
#ifdef DEBUG
          printf("in slave %d, sent back to master\n",mpirank);
#endif /* DEBUG */
...
// gets slave's return and give them a new job until all docking directories have been processed
  if (mpirank == 0) { // master node
#ifdef DEBUG
    printf("Master: waiting to receive slave responses\n");
#endif
    while(fgets(line, LINE_LEN, dockingListFile) != NULL ) {
#ifdef DEBUG
    printf("Master: waiting for chance to submit %s\n",line);
#endif
     if (strlen(line) > 0) {
       char basename[LINE_LEN];
...
// remove eol
       if (line[strlen(line)-1] == '\n')
        ...
        mpirc = MPI_Recv(&slaveDockInfo, sizeof(dockinfo), MPI_BYTE,  MPI_ANY_SOURCE, mpitag, MPI_COMM_WORLD, &mpistatus);
#ifdef PERF_PROFILING
      ...
      time(&slaveDockInfo.masterMainRecvTime_t);
        ...
        if (slaveDockInfo.dockingSuccess == 0) { // slave success
...
// Write out performance profiling info
    ...
    (void) fflush(dockPerformanceFile);	
          ...
          pr(dockErrorStatusFile,"%s failed on node %d - look in stderr for the details, at ", slaveDockInfo.baseName,slaveDockInfo.slaverank);
#endif          
          
          printdate( dockErrorStatusFile, 1 );
         (void) fflush(dockErrorStatusFile);	
        }
      
#ifdef DEBUG
    printf("Master: slave %d returned for docking %s with rc %d\n",slaveDockInfo.slaverank, slaveDockInfo.baseName, slaveDockInfo.dockingSuccess);
#endif
        slaveReturns++;


// prepare to send more work to the same slave
        masterDockInfo.slaverank = slaveDockInfo.slaverank;
        strcpy(masterDockInfo.baseName,line);
#ifdef DEBUG
        printf("Master: about to send dir %s and basename %s\n",dockingDirectoryBaseName,masterDockInfo.baseName);
#endif
	...
	dockingCounter++;
      ...
      time(&masterDockInfo.masterMainSendTime_t);
        ...
        mpirc = MPI_Send(&masterDockInfo, sizeof(dockinfo), MPI_BYTE, slaveDockInfo.slaverank, mpitag, MPI_COMM_WORLD);
        pr(dockSubmittedStatusFile,"%s submitted to node %d at ", line,slaveDockInfo.slaverank);
        printdate( dockSubmittedStatusFile, 1 );
        (void) fflush(dockSubmittedStatusFile);
      
#ifdef DEBUG
      printf("Master: sent to slave %d, mpirc is %d\n",slaveDockInfo.slaverank,mpirc);
#endif /* DEBUG */
     
      }
    }
#ifdef DEBUG
    printf("Master: dropped out of second loop, dockingCounter is %d and slaveReturns is %d\n",dockingCounter,slaveReturns);
#endif

    // wait for remaing slaves to return to master
    while (slaveReturns < dockingCounter) {
#ifdef DEBUG
    printf("Master: waiting for another remaining slave to return\n");
#endif
...
#ifdef DEBUG
    printf("Master: all slaves have returned\n");
#endif

// end all slaves
  for (int i=1;i<mpisize;i++) {
    masterDockInfo.endSlave = 1;
    mpirc = MPI_Send(&masterDockInfo, sizeof(dockinfo), MPI_BYTE, i, mpitag, MPI_COMM_WORLD);
      pr(dockSubmittedStatusFile,"stopping node %d at ", i);
      printdate( dockSubmittedStatusFile, 1 );
      (void) fflush(dockSubmittedStatusFile);
      
  }

  // close all status files
  int close_rc;
    if ( close_rc = fclose(dockSubmittedStatusFile) != 0 ) {
      fprintf(stderr, "\nautodock4 master: can't close %s\n", dockSubmittedStatusFileName);
      exit(-1);
    }
...
//changed these to static and made them threadprivate
static Real crdpdb[MAX_ATOMS][SPACE];  // original PDB coordinates from input
static Real crdreo[MAX_ATOMS][SPACE];  // reoriented coordinates
static Real crd[MAX_ATOMS][SPACE];     // current coordinates
static Quat q_reorient;
static Molecule ligand;        /* ligand */
static Real vt[MAX_TORS][SPACE];
...
static Real eintra = 0.0;  // sum of intramolecular energy for the ligand plus that of the protein
static Real einter = 0.0; // intermolecular energy between the ligand and the protein
static Real unbound_internal_FE = 0.0;
...
//pkcoff - openmp for bluegene change section - start
// moved these search objects to globals and threadprivate in autoglobal.h
// Global_Search *GlobalSearchMethod = NULL;
//Local_Search *LocalSearchMethod = NULL;

// set the threadid and numthreads for each thread
#pragma omp parallel
{
...
#ifdef OPENMP_DEBUG
  printf("\ncall_glss total numThreads is %d this is thread %d",numThreads,threadID);
#endif
...
// pkcoff: change for unduplicated file loading
...
#ifdef AUTODOCK_MPI
#ifdef DEBUG
      printf("in mpi_slave_main for slave %d, argc is %d\n",mpirank,argc);
int argiter;
      for (argiter=0;argiter < argc;argiter++) {
        printf("argv[%d] is %s\n",argiter,argv[argiter]);
      }
#endif /* DEBUG */
#endif 
...
#ifdef AUTODOCK_MPI
#ifdef DEBUG
      printf("setflags failed, returning -1\n");
#endif /* DEBUG */
#endif 
    slaveExit(-1);
} /* END PROGRAM */
...
(void) fprintf(logFile, "                           $Revision: 1.9 $\n\n");
...
#pragma omp parallel
{
        if ((retval==2) || (retval==1)) {
		    ...
		    // pkcoff - for openmp the process id seed won't work since it will be duplicated among all the threads
		    
                    // set the seed based on the seed type
                    if (seedtype == SAME_SEED) {
                      seed[i] = SAME_SEED_CONSTANT;
                       pr(logFile,"Random number generator was seeded with the constant value = %ld\n",seed[i]);
                    }
                    else if (seedtype == DEFAULT_SEED) { // the time seed needs to have the threadID added to it so that different threads get a different seed
                      seed[i] = (FourByteLong)(time( &time_seed ) + threadID);
                       pr(logFile,"PID %d Thread %d iter %d Random number generator was seeded with the current time, value = %ld\n",getpid(), threadID, i, seed[i]);
                    }
                    else if (seedtype == UNIQUE_NODE_SEED) { // the time seed needs to have the threadID added to it so that different threads get a different seed plus the mpirank times the most threads possible on BQG (320) to make sure the seed is unique across the nodes
                      seed[i] = (FourByteLong)(time( &time_seed ) + threadID + (mpirank*320));
                       pr(logFile,"Random number generator was seeded with the current time + mpirank, value = %ld\n",seed[i]);                      
                    }
                    else {
...
/*____________________________________________________________________________*/
    ...
    case DPF_LIGAND_TYPES:
        ...
        if (num_atom_types > 0) {
            ...
            prStr( error_message, "%s:  msg 1 ERROR! No atom types have been found; we cannot continue without this information!\n\n", programname );
            pr_2x( logFile, stderr, error_message );
...
//______________________________________________________________________________
    ...
    serialStartClock = (Real) times(&tms_serialStart);
        ...
        */
        ...
        // pkcoff: code to enable unduplicated file loading
        if (mapusagetype == RELOAD_MAPS)
          num_maps = 0;
                        
// printf("readfld: before num_maps is %d and info->spacing is %f\n",num_maps, info->spacing);                
...
// printf("readfld: after num_maps is %d and info->spacing is %f\n",num_maps, info->spacing);                
...
// printf("FileName is %s and loadedMapFileNames[%d] is %s and atomTypeMap[%d] is %s\n",FileName,i,loadedMapFileNames[i],i,atomTypeMap[i]);
...
// printf("mapFileAlreadyLoaded is %d\n",mapFileAlreadyLoaded);
            ...
            map_stats = readmap( line, outlev, jobStart, tms_jobStart, B_charMap, &B_havemap, num_maps, info, map, 'a',mapFileAlreadyLoaded, mapusagetype );
            pr(logFile, "Min= %.3lf Mean= %.3lf Max= %.3lf\n\n",
                    map_stats.minimum, map_stats.mean, map_stats.maximum);
            }

// printf("after readmap\n");

            if (!mapFileAlreadyLoaded) {
              num_maps++;
            }
// printf("after readmap mapFileAlreadyLoaded\n");

        } else {
            prStr( error_message, "%s:  msg 2 ERROR! No atom types have been found; we cannot continue without this information!\n\n", programname );
            pr_2x( logFile, stderr, error_message );
            prStr( error_message, "%s:  ERROR! Are you trying to use an AutoDock 3 DPF with AutoDock 4?\n\n", programname );
            pr_2x( logFile, stderr, error_message );
            slaveExit(-1);
...
// printf("FileName is %s and loadedMapFileNames[%d] is %s\n",FileName,i,loadedMapFileNames[i]);
            ...
            if (!mapFileAlreadyLoaded) {
              map_stats = readmap( line, outlev, jobStart, tms_jobStart, B_charMap, &B_havemap, num_maps, info, map, 'e', mapFileAlreadyLoaded, mapusagetype );
              pr(logFile, "Min= %.3lf Mean= %.3lf Max= %.3lf\n\n",
                map_stats.minimum, map_stats.mean, map_stats.maximum);
              ElecMap = num_maps;
        }
...
// printf("mapFileAlreadyLoaded is %d\n",mapFileAlreadyLoaded);
            ...
            if (!mapFileAlreadyLoaded) {
              ...
              map_stats = readmap( line, outlev, jobStart, tms_jobStart, B_charMap, &B_havemap, num_maps, info, map, 'd', mapFileAlreadyLoaded, mapusagetype );
              pr(logFile, "Min= %.3lf Mean= %.3lf Max= %.3lf\n\n",
                map_stats.minimum, map_stats.mean, map_stats.maximum);
              DesolvMap = num_maps;
           }
...
//______________________________________________________________________________
 ...
 //printf("mapFileAlreadyLoaded is %d\n",mapFileAlreadyLoaded);
            ...
            map_stats = readmap( line, outlev, jobStart, tms_jobStart, B_charMap, &B_havemap, num_maps, info, map, 'c', mapFileAlreadyLoaded, mapusagetype );
            pr(logFile, "Min= %.3lf Mean= %.3lf Max= %.3lf\n\n",
                    map_stats.minimum, map_stats.mean, map_stats.maximum);
            }
...
/*____________________________________________________________________________*/
    ...
    case DPF_COLINY:
    {
        ...
        if (strcmp(algname,"help")==0) {
            ...
            evaluate[threadID].setup( crd, charge, abs_charge, qsp_abs_charge, type, natom,
...
//pkcoff - openmp for bluegene change section start
// need to create global search object for all the threads
#pragma omp parallel
{
...
//pkcoff - openmp for bluegene change section start
// need to create local search object for all the threads
#pragma omp parallel 
{
...
//pkcoff - openmp for bluegene change section start
// need to create local search object and rho arrays for all the threads
#pragma omp parallel
{
...
//pkcoff - openmp for bluegene change section - start
// these are all static variables in mpi_slave_main that are set in the master thread while parsing the various sections of the dpf, every thread needs to be initialized with the values
// this parallel section calls the setup method on the evaluate object indexed by the threadID
//#pragma omp parallel copyin(crd, crdreo, vt, crdpdb, q_reorient, ligand,map)
#pragma omp parallel copyin(crd, crdreo, vt, crdpdb, q_reorient, ligand)
{
        /*
        ...
        */
            ...
            sprintf(searchResultsLine[threadID],"Number of requested LGA dockings = %d run%c\n", nruns, (nruns > 1)?'s':' ');
            strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);
            
            if (ad4_unbound_model==Unbound_Default) ad4_unbound_model = Unbound_Same_As_Bound;
            sprintf(searchResultsLine[threadID],"Unbound model to be used is %s.\n", report_parameter_library());
            strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);


#ifdef DEBUG
            pr( logFile, "\nAbout to call evaluate[threadID].setup(), sInit:\n\n");
...
//             printf("Number of runs: %d pop_size is %d\n",nruns,pop_size);

// nconf_before_lga is used to maintain the proper index into the sHist array during the for-loop parallelism - wherever you see 'nconf_before_lga+j' as an index this value used to be just nconf in the serial code
	{
            int nconf_before_lga = nconf;
            
// this for-loop is the heart of the algorithm which takes the majority of the docking time performing local searches
// the number of runs (nruns) are set explicitly in the dpf file with the ga-runs keyword
// the output of the loops is a full sHist State array of conformations
// all loops are independant, each local search is basically a completely new sample search
// for the copyin clause they are all static variables in mpi_slave_main that are set in the master thread while parsing the various sections of the dpf, every thread needs to be initialized with the values
// optimum parallelism is achieved if the number of nruns is evenly divisible by the number of threads
// the private clause are the timing and energy computation variables which are computed for every iteration
...
#pragma omp parallel for private(gaStart, tms_gaStart, gaEnd, tms_gaEnd)  copyin(crd, crdreo, vt, crdpdb, q_reorient, ligand, eintra, einter,unbound_internal_FE)
                        
            for (int j=0; j<nruns; j++) {

// printf("LGA start for iter %d\n",j);

                j1=j+1;
		char dateBuffer[100];
	     	strcpy(dateBuffer,"");
	     	strcpy(searchResultsBuffer[threadID],"");
		printdate_buffer(2);
	     	strncpy(dateBuffer,searchResultsBuffer[threadID],100);
	     	strcpy(searchResultsBuffer[threadID],"");
                ...
                if (B_reorient_random == TRUE) {
                     ...
                     evaluate[threadID].update_crds( crdpdb, vt );
                 }
                 ...
                 sHist[nconf_before_lga+j] = call_glss( GlobalSearchMethod, LocalSearchMethod,
                                           sInit,
                                           num_evals, pop_size,
                                           outlev,
		    ...
		    strcat(searchResultsBuffer[threadID], "\nRun completed;  time taken for this run:\n");
                    timesyshms_buffer( gaEnd - gaStart, &tms_gaStart, &tms_gaEnd );
                                       
                   
                    strcat(searchResultsBuffer[threadID], "\n");
                    printdate_buffer( 1 );
		    sprintf(searchResultsLine[threadID],"Total number of Energy Evaluations for thread ID %d: %lu\n",threadID, evaluate[threadID].evals());
		    strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);
		    sprintf(searchResultsLine[threadID],"Total number of Generations:        %u\n", ((Genetic_Algorithm *)GlobalSearchMethod)->num_generations());
		    strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);
		    ...
		    strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);
                 ...
                 writePDBQT( j, seed,  FN_ligand, dock_param_fn, lig_center,
                             sHist[nconf_before_lga+j], ntor, &eintra, &einter, natom, atomstuff,
                             crd, emap, elec,
                             charge, abs_charge, qsp_abs_charge,
                             ligand_is_inhibitor,
                             torsFreeEnergy,
                 ...
                 econf[nconf_before_lga+j] = eintra + einter + torsFreeEnergy - unbound_internal_FE;
 
                 ++nconf;
 
		    sprintf(searchResultsLine[threadID],UnderLine);
		    strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);
                 ...
                 #pragma omp critical 
		 {
                 ...
                 Real curentLGARunTime = (Real)(gaEnd - gaStart) * idct;
// printf("curentLGARunTime is %.2lfs %05.2lfs\n", (double) curentLGARunTime, (double) curentLGARunTime);

                 if (slaveDockInfo.LGAMaxRuntime < curentLGARunTime)
                   slaveDockInfo.LGAMaxRuntime = curentLGARunTime;
                 if ((slaveDockInfo.LGAMinRuntime > curentLGARunTime) || (slaveDockInfo.LGAMinRuntime == -1))
                   slaveDockInfo.LGAMinRuntime = curentLGARunTime;
                 slaveDockInfo.LGAIters++;
                 slaveDockInfo.LGATotalRuntime += curentLGARunTime;
                 }
#endif
		 ...
		 #pragma omp critical 
		 {
		    ...
		    pr( logFile, searchResultsBuffer[threadID]);
	    	    (void) fflush(logFile);
	    	    strcpy(searchResultsBuffer[threadID],"");
                   ...
                   Real curentLGARunTimeIO = (Real) (serialEndClock-serialStartClock) * idct;;  
//printf("curentLGARunTimeIO time is %.2lfs\n", (double) curentLGARunTimeIO);

                 if (slaveDockInfo.LGAMaxRuntimeIO < curentLGARunTimeIO)
                   slaveDockInfo.LGAMaxRuntimeIO = curentLGARunTimeIO;
                 if ((slaveDockInfo.LGAMinRuntimeIO > curentLGARunTimeIO) || (slaveDockInfo.LGAMinRuntimeIO == -1))
                   slaveDockInfo.LGAMinRuntimeIO = curentLGARunTimeIO;
                 slaveDockInfo.LGAItersIO++;
                 slaveDockInfo.LGATotalRuntimeIO += curentLGARunTimeIO;
    ...
    process_DPF_COMPUTE_UNBOUND_EXTENDED:
      ...
      time(&slaveDockInfo.UnboundExtendedStartTime_t);
	    ...
	    strcpy(searchResultsBuffer[0],"");
	    	...
	    	pr( logFile, searchResultsBuffer[0]);
	    	(void) fflush(logFile);
