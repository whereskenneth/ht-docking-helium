/*
 ...
 $Id: support.cc,v 1.3 2010/07/30 23:51:12 pkcoff Exp $
...
double Phenotype::evaluate(EvalMode mode)
{
   ...
   {
      case Always_Eval:
         value = ::evaluate[threadID](value_vector);
         evalflag = 1;
         break;
      case Always_Eval_Nonbond:
         value = ::evaluate[threadID](value_vector, 1); // term=1 as total non-bonded energy, i.e. vdW+Hb+desolv
         evalflag = 0;
         break;
      case Always_Eval_Elec:
         value = ::evaluate[threadID](value_vector, 2); // term=2 as total electrostatic energy
         evalflag = 0;
         break;
      case Normal_Eval:
         if (!evalflag) {
