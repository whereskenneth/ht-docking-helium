/*
 ...
 $Id: stateLibrary.cc,v 1.2 2010/04/01 15:37:13 pkcoff Exp $
...
void printState_buffer( State S, 
                 int detail )
{
    register int i;
    Real torDegTmp;
    ...
    switch( detail ) {
	    ...
	    sprintf(searchResultsLine[threadID],"\nSTATE VARIABLES:\n________________\n\nTranslation x,y,z         = %.3f %.3f %.3f\nQuaternion x,y,z,w        = %.3f %.3f %.3f %.3f\n",S.T.x, S.T.y, S.T.z,S.Q.x, S.Q.y, S.Q.z, S.Q.w);
	    strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);
            
            
            S.Q = convertQuatToRot( S.Q );
            S.Q.ang = WrpRad( ModRad( S.Q.ang ));
	    sprintf(searchResultsLine[threadID],"Axis-Angle nx,ny,nz,angle = %.3f %.3f %.3f %.3f\n", S.Q.nx, S.Q.ny, S.Q.nz, RadiansToDegrees(S.Q.ang) );
	    strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);


            //(void)fprintf( fp, "Quaternion qmag           = %.3f\n", S.Q.qmag );
	    sprintf(searchResultsLine[threadID],"Number of Torsions        = %d\n", S.ntor );
	    strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);
            if (S.ntor > 0) {
                sprintf(searchResultsLine[threadID],"Torsions (degrees)        =");
	        strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);
                for (i=0; i<S.ntor; i++) {
                    S.tor[i] = WrpRad( ModRad( S.tor[i] ) );
                }
                for (i=0; i<S.ntor; i++) {
                    ...
                    torDegTmp = ModDeg( torDegTmp );
                    torDegTmp = WrpDeg( torDegTmp );
                    sprintf(searchResultsLine[threadID]," %.2f", torDegTmp );
	            strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);
                    //if ((B_isTorConstrained[i] == 1) && B_ShowTorE) {
                        //pr( fp, ", Energetic penalty = %uhd\n", US_TorE[i]);
                    //} else {
        ...
        case 3:
            // Writes only the translation component of the state
            sprintf(searchResultsLine[threadID],"%.3f %.3f %.3f", S.T.x, S.T.y, S.T.z );
            strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);
            break;
    }
}

void writeState_buffer( State S )
{
    register int i;
    Real torDegTmp;
    ...
    // Write translation.
    sprintf(searchResultsLine[threadID],"%7.3f %7.3f %7.3f  ", S.T.x, S.T.y, S.T.z );    strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);
    ...
    // Write axis-angle.
    S.Q.ang = WrpRad( ModRad( S.Q.ang ));
    sprintf(searchResultsLine[threadID],"%6.3f %6.3f %6.3f %6.3f  ", S.Q.nx, S.Q.ny, S.Q.nz, RadiansToDegrees(S.Q.ang) );
    strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);
    ...
    if (S.ntor > 0) {
        ...
        for (i=0; i<S.ntor; i++) {
            ...
            // numbers are space-delimited.
            //pr( fp, " %.2f%c", torDegTmp, (i==(S.ntor-1) ? '.' : ','));
	    sprintf(searchResultsLine[threadID]," %7.2f", torDegTmp );
	    strcat(searchResultsBuffer[threadID], searchResultsLine[threadID]);
        }
    }
    // Leave fp on this line for energies which follow....
